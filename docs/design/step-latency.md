...
| 配置 | 追加的通信 |
| --- | --- |
| TP > 1 | 每层 all-reduce，消息为 tokens × hidden |
| attention DP > 1 | MoE 前 all-gather、MoE 后 reduce-scatter，或由 all-to-all 取代 |
| 宽 EP | dispatch 与 combine 的 all-to-all，node_num = ceil(ep × tp / 每节点器件数) |
| PP | 级间 P2P，次数为 pp − 1 |
...
每步另加固定开销 t_step_fixed，来自 stack 配置，表示 kernel launch、host 调度、采样和整步 graph 重放是否可用带来的差别。算子实测表是在 CUDA Graph 重放下采集的，不含这部分。
...
MTP 由 stack 配置给出草稿长度 nextn 和接受长度的分布。decode 的有效 batch 乘以 (nextn + 1)；草稿层的代价按 generation 算子的 (nextn + L)/L 倍计入；每个请求每步前进 1 + accepted 个 token。接受长度必须由配置给出，DLsim 不内置接受率。
